...
# bucket = The name of the bucket that the request was processed against. If the system receives a malformed request and cannot determine the bucket, the request will not appear in any server access log.
# time = The time at which the request was received. The format, using strftime() terminology, is [%d/%b/%Y:%H:%M:%S %z]
# remote_ip = The apparent Internet address of the requester. Intermediate proxies and firewalls might obscure the actual address of the machine making the request
# requester = The canonical user id of the requester, or the string "Anonymous" for unauthenticated requests. This identifier is the same one used for access control purposes
# request_id = The request ID is a string generated by Amazon S3 to uniquely identify each request
# operation = Either SOAP.operation, REST.HTTP_method.resource_type or WEBSITE.HTTP_method.resource_type
#       Ex.
#       REST.POST.BUCKET -- upload
#       REST.OPTIONS.PREFLIGHT -- ?
#       REST.PUT.OBJECT -- Add logs
#       REST.DELETE.OBJECT -- Remove object
# key = The "key" part of the request, URL encoded, or "-" if the operation does not take a key parameter
# request_uri = The Request-URI part of the HTTP request message
# http_status = The numeric HTTP status code of the response
# error_code = The Amazon S3 Error Code, or "-" if no error occurred (http://docs.aws.amazon.com/AmazonS3/latest/dev/ErrorCode.html)
...
# total_time = The number of milliseconds the request was in flight from the server's perspective. This value is measured from the time your request is received to the time that the last byte of the response is sent. Measurements made from the client's perspective might be longer due to network latency
# turn_around_time = The number of milliseconds that Amazon S3 spent processing your request. This value is measured from the time the last byte of your request was received until the time the first byte of the response was sent
...
# versio_id = The version ID in the request, or "-" if the operation does not take a versionId parameter
#
# Log entry Example:
